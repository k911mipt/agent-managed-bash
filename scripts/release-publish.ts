...
async function reconcileNpm(candidate: PublicationCandidate, npm: string, bootstrapRequested: boolean, bootstrapAllowed: boolean): Promise<void> {
  ...
  if (current === undefined) {
    ...
    await mutateThenRead(npm, ["publish", resolve(tarball.path), "--access", "public", "--provenance", "--dry-run=false"], async () => readNpm(candidate, npm, tarball), token === undefined ? {} : { NODE_AUTH_TOKEN: token })
    return
  }
